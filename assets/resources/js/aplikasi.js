$(document).ready(function() {
	...
	if (url == "siswa") {
		pagination("datatabel", base_url+"adm/siswa/data", []);
	} else if (url == "guru") {
		pagination("datatabel", base_url+"adm/guru/data", []);		
	} else if (url == "mata_pelajaran") {
		pagination("datatabel", base_url+"adm/mata_pelajaran/data", []);		
	} else if (url == "soal_ujian") {
		pagination("datatabel", base_url+"adm/soal_ujian/data", []);
...
function pagination(indentifier, url, config) {
    $('#'+indentifier).DataTable({
        "language": {
            "url": base_url+"assets/resources/plugin/datatables/Indonesian.json"
        },
        "ordering": false,
        "columnDefs": config,
        "bProcessing": true,
...
function soal_ujian_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/soal_ujian/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/soal_ujian"); 
				} else {
					console.log('gagal');
				}
			}
...
function siswa_s() {
	...
	$.ajax({		
		type: "POST",
		url: base_url+"adm/siswa/simpan",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
		if (response.status == "ok") {
			window.location.assign(base_url+"adm/siswa"); 
		} else {
			console.log('gagal');
		}
	});
...
function siswa_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/siswa/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/siswa"); 
				} else {
...
function siswa_u(id) {
	if (confirm('Anda yakin..? Username dan Password otomatis adalah NIM ..!')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/siswa/user/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/siswa"); 
				} else {
...
function siswa_ur(id) {
	if (confirm('Anda yakin..? Username dan Password otomatis adalah NIM ..!')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/siswa/user_reset/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/siswa"); 
				} else {
...
function siswa_non_aktif(id) {
	if (confirm('Anda yakin akan menonaktifkan user ini..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/siswa/non_aktifkan/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/siswa"); 
				} else {
...
function guru_s() {
	...
	$.ajax({		
		type: "POST",
		url: base_url+"adm/guru/simpan",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
		if (response.status == "ok") {
			window.location.assign(base_url+"adm/guru"); 
		} else {
			console.log('gagal');
		}
	});
...
function guru_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/guru/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/guru"); 
				} else {
...
function guru_ur(id) {
	if (confirm('Anda yakin..? Username dan Password otomatis adalah NIP ..!')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/guru/user_reset/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/guru"); 
				} else {
...
function guru_matkul(id) {
	$.ajax({
		type: "GET",
		url: base_url+"adm/guru/ambil_matkul/"+id,
		success: function(data) {
			if (data.status == "ok") {
				var jml_data	= Object.keys(data.data).length;
				var hate 	= '<div class="modal fade" id="siswa_matkul" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting Mata Kuliah</h4></div><div class="modal-body"><form name="f_siswa_matkul" id="f_siswa_matkul" method="post" onsubmit="return guru_matkul_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="'+id+'"><div id="konfirmasi"></div>';
...
function guru_matkul_s() {
	var f_asal	= $("#f_siswa_matkul");
	var form	= getFormData(f_asal);
	$.ajax({		
		type: "POST",
		url: base_url+"adm/guru/simpan_matkul",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
...
function mata_pelajaran_e(id) {
	$("#mata_pelajaran").modal('show');
	$.ajax({
		type: "GET",
		url: base_url+"adm/mata_pelajaran/det/"+id,
		success: function(data) {
			$("#id").val(data.id);
			$("#nama").val(data.nama);
			$("#nama").focus();
...
function mata_pelajaran_s() {
	var f_asal	= $("#f_mapel");
	var form	= getFormData(f_asal);
	$.ajax({		
		type: "POST",
		url: base_url+"adm/mata_pelajaran/simpan",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
		if (response.status == "ok") {
			window.location.assign(base_url+"adm/mata_pelajaran"); 
		} else {
			console.log('gagal');
		}
	});
...
function mata_pelajaran_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/mata_pelajaran/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/mata_pelajaran"); 
				} else {
